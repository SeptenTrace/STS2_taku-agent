...
export async function dispatch(client: RequestClient, command: string, args: string[]): Promise<void> {
  switch (command) {
    ...
    case "--help":
      ...
      return;
    case "ping":
      await printRequest(client, "/");
      return;
    case "capabilities":
      await printRequest(client, "/api/v1/capabilities");
      return;
    case "context":
      await printRequest(client, "/api/v1/context");
      return;
    case "next":
      await commandNext(client);
      return;
    case "compact":
    case "observe":
    case "observation":
      await printRequest(client, "/api/v1/observation/compact");
      return;
    case "delta":
      await printRequest(client, "/api/v1/observation/delta");
      return;
    case "actions":
      await printRequest(client, "/api/v1/actions");
      return;
    case "run":
      await printRequest(client, "/api/v1/run");
      return;
    case "knowledge":
      await commandFromMap(client, args[0], knowledgePaths, "knowledge");
      return;
    case "player":
      await commandFromMap(client, args[0], playerPaths, "player");
      return;
    case "combat":
      await commandFromMap(client, args[0], combatPaths, "combat");
      return;
    case "map":
      await printRequest(client, "/api/v1/map/summary");
      return;
    case "event":
      await printRequest(client, "/api/v1/event");
      return;
    case "shop":
      await printRequest(client, "/api/v1/shop");
      return;
    case "rest":
    case "rest-site":
      await printRequest(client, "/api/v1/rest-site");
      return;
    case "rewards":
      await printRequest(client, "/api/v1/rewards");
      return;
    case "card-reward":
      await printRequest(client, "/api/v1/card-reward");
      return;
    case "card-select":
    case "card-selection":
      await printRequest(client, "/api/v1/card-selection");
      return;
    case "treasure":
      await printRequest(client, "/api/v1/treasure");
      return;
    case "wait": {
      ...
      printJson(await waitForCondition(client, condition, timeout));
      return;
    }
    case "exec":
    case "do":
    case "act": {
      ...
      printJson(await client.request("/api/v1/actions/execute", { method: "POST", body: payload }));
      return;
    }
    case "full":
      await printRequest(client, "/api/v1/state/full");
      return;
    case "get": {
      const path = args[0];
      if (!path) {
